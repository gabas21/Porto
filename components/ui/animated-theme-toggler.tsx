...
export const AnimatedThemeToggler = ({
  ...
  const toggleTheme = useCallback(() => {
    ...
    // Animasi asimetris:
    // • Dark -> Light: "Berkumpul" (inward) — snapshot gelap lama menyusut ke titik tombol
    // • Light -> Dark: "Menyebar" (outward) — layer gelap baru meluas keluar dari titik tombol
    const isInward = isDark;

    const applyTheme = () => {
      const newTheme = !isDark;
      const themeString = newTheme ? "dark" : "light";
      
      soundFx.playThemeToggle(newTheme);
    ...
    root.dataset.magicuiThemeDirection = isInward ? "inward" : "outward";
    ...
    root.style.setProperty(
      ...
      isInward ? clipPath[1] : clipPath[0]
    );
    ...
    if (ready && typeof ready.then === "function") {
      ready
        .then(() => {
          const animClipPath = isInward
            ? [clipPath[1], clipPath[0]] // MaxRadius -> 0% (Menyusut/Berkumpul ke tombol)
            : [clipPath[0], clipPath[1]]; // 0% -> MaxRadius (Menyebar keluar dari tombol)

          const targetPseudoElement = isInward
  ...
  return (
    ...
    >
      {isDark ? (
        ...
        <Moon className="h-4 w-4 text-sky-500 transition-transform duration-200" />
      )}
